...
ACTION_PREDICTION_TARGETS: Final[Mapping[str, frozenset[str]]] = {
    ...
    "nutrient_ec_check": frozenset(
        {"root_zone_ec", "drainage_ec", "feed_ec", "substrate_ec", "ec"}
    ),
    "ph_check": frozenset({"root_zone_ph", "drainage_ph", "feed_ph", "ph"}),
    "ventilation_dehumidification": frozenset(
        {"humidity_pct", "vpd_kpa", "temperature_c", "leaf_wetness_hours"}
    ),
    "shading_high_temperature": frozenset(
        {"temperature_c", "vpd_kpa", "radiation_w_m2", "cumulative_radiation_j_cm2"}
    ),
    "heating_low_temperature": frozenset({"temperature_c", "outside_temperature_c"}),
    "disease_environment_risk_proxy": frozenset(
        {"humidity_pct", "vpd_kpa", "rain_probability_pct", "leaf_wetness_hours"}
    ),
